...
def filter_triplets(tp, min_uc=5, min_sc=0):
    # Only keep the triplets for items which are clicked on 
    # by at least min_sc users.
    if min_sc > 0:
    ...
    if min_uc > 0:
        ...
        tp = tp[tp['userId'].isin(itemcount.index[itemcount >= min_uc])]
