...
def setup_train(args):
    ...
    pp.pprint({k: v for k, v in vars(args).item() if v is not None}, width=1)
    return export_root
...
def create_experiment_export_folder(args):
    ...
    experiment_path = get_name_of_experiement_path(experiment_dir, experiment_description)
    os.mkdir(experiment_path)
    print('Folder created: ' + os.path.abspath(experiment_path))
    return experiment_path


def get_name_of_experiement_path(experiment_dir, experiment_description):
